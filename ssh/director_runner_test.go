...
var _ = Describe("director ssh runner", func() {
	...
	It("runs the script with an ssh to the director vm", func() {
		...
		Expect(lines).To(Equal([]string{
			...
			`director_ssh_key="$(OM_CLIENT_ID='client_id' OM_CLIENT_SECRET='client_secret' OM_USERNAME='username' OM_PASSWORD='password' om -t https://www.test-url.io -k curl -s -p /api/v0/deployed/director/credentials/bbr_ssh_credentials | jq -r .credential.value.private_key_pem)"`,
